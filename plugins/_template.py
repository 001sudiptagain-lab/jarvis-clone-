"""
Drop-in ZYROX plugin template.

Copy this file, rename it (no leading underscore), fill in PLUGIN and run().
No other file needs to change — ZYROX discovers this automatically at startup.
"""
...
def run(parameters: dict, player=None, session_memory=None) -> str:
    ...
    player: the ZyroxUI instance — use player.write_log(f"ZYROX: ...") to log,
    ...
    if player:
        try:
            player.write_log(f"ZYROX: {result_text}")
        except Exception:
            pass
    return result_text
