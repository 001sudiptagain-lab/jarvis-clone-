...
class LogWidget(QTextEdit):
    ...
    def __init__(self, parent=None):
        ...
        self._pos     = 0
        self._tag     = "sys"
        self._ai_name_lc = "zyrox"   # updated when assistant name changes
        self._tmr = QTimer(self)
        self._tmr.timeout.connect(self._step)
    ...
    def _next(self):
        ...
        elif tl.startswith(_ai_pfx) or tl.startswith("zyrox:") or tl.startswith("jarvis:"): self._tag = "ai"
...
class FileDropZone(QWidget):
    ...
    def _browse(self):
        path, _ = QFileDialog.getOpenFileName(
            self, "Select a file for ZYROX", str(Path.home()),
            "All Files (*.*);;"
...
class CustomizeOverlay(QWidget):
    ...
    def __init__(self, assistant_name="ZYROX", user_name="",
    ...
    def _save(self):
        name = self._name_input.text().strip() or "ZYROX"
...
class AudioDeviceOverlay(_HudOverlay):
    ...
    def __init__(self, parent=None):
        ...
        self._in_box  = _row(f"MICROPHONE — what {self._assistant_name} hears you with",
                             "input", get_input_device())
        lay.addSpacing(4)
        self._out_box = _row(f"SPEAKERS — what {self._assistant_name} talks through",
...
class MemoryOverlay(_HudOverlay):
    ...
    def _rebuild(self):
        ...
        hdr = QLabel(f"🧠  WHAT {self._assistant_name} REMEMBERS")
        hdr.setFont(QFont("Courier New", 12, QFont.Weight.Bold))
        hdr.setStyleSheet(f"color: {C.PRI}; background: transparent;")
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, face_path: str):
        ...
        self._assistant_name: str = (_cfg.get("assistant_name") or "ZYROX").strip()
    ...
    def _build_header(self) -> QWidget:
        ...
        _sub_text = ("A Friendly Assistant"
                     if _disp in ("ZYROX", "JARVIS", "J.A.R.V.I.S")
    ...
    def _refresh_hud_btn(self):
        ...
        self._hud_btn.setToolTip(
            "An animated head that speaks your words and shows what ZYROX is "
    ...
    def _open_customize(self):
        ...
        cw = self.centralWidget()
        ov = CustomizeOverlay(
            cfg.get("assistant_name", "ZYROX") or "ZYROX",
            cfg.get("user_name", ""),
    ...
    def _apply_name_update(self, name: str, user_name: str, ui_color: str = "",
        ...
        self._assistant_name = name.strip() or "ZYROX"
        display = self._assistant_name.upper()
        self.setWindowTitle(f"{display} — {APP_VERSION}")
        self._title_lbl.setText(display)
        if display in ("ZYROX", "JARVIS", "J.A.R.V.I.S"):
    ...
    def _on_setup_done(self, key: str, os_name: str):
        ...
        if self._overlay:
            ...
            self._overlay = None
        self._apply_state("LISTENING")
        self._assistant_name = _read_full_config().get("assistant_name", "ZYROX") or "ZYROX"
