...
def _make_uploads_dir() -> Path:
    ...
    for candidate in [
        Path.home() / "Downloads" / "ZYROX Uploads",
        Path.home() / "Documents" / "ZYROX Uploads",
        BASE_DIR / "uploads",
    ]:
        try:
...
def _ensure_network_access(port: int) -> None:
    ...
    if sys.platform == "win32":
        ...
        port_rule = f"ZYROX Dashboard Port {port}"
        prog_rule  = "ZYROX Dashboard Python"
        ...
        try:
            ...
            if int(ret) > 32:
                ...
                print("[Dashboard] Phone connections may fail until ZYROX is run as Administrator.")
...
def _ensure_certs() -> bool:
    ...
    try:
        ...
        who = x509.Name([
            x509.NameAttribute(NameOID.COMMON_NAME, "ZYROX Dashboard"),
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, "ZYROX"),
        ])
...
<p>Connecting to ZYROX…</p>
    ...
    async def serve(self) -> None:
        ...
        print("[Dashboard] Press 'Remote Control' in ZYROX UI to get the QR code.")
        await uvicorn.Server(cfg).serve()
