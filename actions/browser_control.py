...
class _BrowserSession:
    ...
    async def _launch(self):
        ...
        if engine_name == "firefox":
            ...
            try:
                ...
                print(f"[Browser] Firefox real profile failed ({e}), using ZYROX profile")
        ...
        # automation). Fall back to a persistent ZYROX automation profile —
        ...
        print(f"[Browser] Retrying with ZYROX profile: {jarvis_profile}")

        try:
            self._context = await engine_obj.launch_persistent_context(jarvis_profile, **kwargs)
            self._page = await self._adopt_page()
            print(f"[Browser] ✅ Launched [{label}] with ZYROX profile "
    ...
    async def screenshot(self, path: str = None) -> str:
        page = await self._get_page()
        try:
            save_path = path or str(Path.home() / "Desktop" / "zyrox_screenshot.png")
