...
def _cancel_scheduled_update() -> str:
    if is_windows():
        result = subprocess.run(
            ["schtasks", "/Delete", "/TN", "ZYROX_GameUpdater", "/F"],
            capture_output=True, text=True, **_CNW
        )
...
def _get_schedule_status() -> str:
    if is_windows():
        result = subprocess.run(
            ["schtasks", "/Query", "/TN", "ZYROX_GameUpdater", "/FO", "LIST"],
            capture_output=True, text=True, **_CNW
        )
        if result.returncode != 0:
    ...
    try:
        ...
        if "ZYROX_GameUpdater" in result.stdout:
            for line in result.stdout.splitlines():
                if "ZYROX_GameUpdater" in line:
